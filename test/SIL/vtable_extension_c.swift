...
// CHECK: [[REF:%.*]] = class_method %0 : $@thick Base.Type, #Base.inBEC!1 : (Base.Type) -> () -> (), $@convention(method) (@thick Base.Type) -> ()
// CHECK-NEXT: apply [[REF]](%0) : $@convention(method) (@thick Base.Type) -> ()

func invoke(baseStaticExtension b: Base.Type) {
  b.inBES()
}
// CHECK: [[REF:%.*]] = function_ref @_T04main4BaseC5inBESyyFZ : $@convention(method) (@thick Base.Type) -> ()
// CHECK-NEXT: apply [[REF]](%0) : $@convention(method) (@thick Base.Type) -> ()

func invoke(finalBaseExtension b: Base) {
  b.inBEF()
}
// CHECK: [[REF:%.*]] = function_ref @_T04main4BaseC5inBEFyyF : $@convention(method) (@guaranteed Base) -> ()
// CHECK-NEXT: apply [[REF]](%0) : $@convention(method) (@guaranteed Base) -> ()
